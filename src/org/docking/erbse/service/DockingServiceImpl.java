...
public class DockingServiceImpl implements DockingService 
{
   ...
   public String getEditorCode(String editorId, String path) {
      ...
      String realPath = (editorId  + "\\" +  path).replace("/", "\\");
      ...
      EditorCodeVO ecvo = ecvoService.search("editorCode_search", realPath);
      code = ecvo.getCode();
      ...
      DockingStream ds = new SingleDataUpdateFilter(new NextDataTargetInsertFilter(new SingleDataUpdateFilter(new DataRegister(code),new byte[][]{"../".getBytes()},new byte[][]{"./".getBytes()}),srcData,rsrcData,insertData,false),targetData,processData);
      Map<Integer, Attribute> attrMap = ds.analyze();
      
      DataAttribute dAttr = (DataAttribute) attrMap.get(Attr.DATA_ATTR);      
      ...
      if(eeivo.getStartPage().equals(path)){
         ...
         ds = new SingleDataUpdateFilter(new DataRegister(code),new byte[][]{"<body>".getBytes()},new byte[][]{scriptSrc.getBytes()});
         Map<Integer, Attribute> mapAttr = ds.analyze();
         dAttr = (DataAttribute)mapAttr.get(Attr.DATA_ATTR);
         code = dAttr.getStringValue()[0];
      }
